...
- [x] **Step 1.3** - Verificar Node/Next versão: `node -v && npx next --version` (esperado: Next 16.2.9)
...
- [x] **Step 2.3** - Criar `src/app/api/auth/refresh-role/route.ts`:
  ...
  ```
- [x] **Step 2.4** - Editar `src/app/api/users/route.ts` (PUT): após `dbSim.updateUserHierarchy`, chamar `fetch('/api/auth/refresh-role', { method: 'POST', body: JSON.stringify({ userId }) })`
...
- [x] **Step 3.2** - Ajustar import em `src/auth.ts` se houver referência cruzada
- [x] **Step 3.3** - Verificar `next.config.ts` não tem config `middleware:` antiga
...
- [x] **Step 4.3** - Testar visual: `npm run dev` -> página login -> verificar animação suave, sem travamento, 60fps (DevTools Performance)
...
- [x] **Step 5.3** - Verificar se `src/app/layout.tsx` mantém `<SessionProvider>` na raiz
...
- [x] **Step 7.1** - Cold start: `npm run dev` -> abrir `http://localhost:3000` -> medir tempo até "Ready in Xms" no terminal
...
- [x] **Step 7.5** - Build produção: `npm run build` -> verificar sem erros
