...
export default function ComunicadosClient({
  ...
  const reloadAnnouncements = async () => {
    try {
      ...
      console.error("Erro ao recarregar comunicados:", err);
    }
  };
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      {...register("title")}
                      placeholder="Ex: Manutencao programada, Nova politica, Resultado trimestral"
        ...
        <div className="text-center py-12 border border-dashed border-brand-terciar/20 bg-white rounded-2xl shadow-sm">
          ...
          <p className="text-sm text-brand-terciar/50">
            {search || activeFilter !== "ALL"
              ...
              : 'Nenhum comunicado cadastrado. Clique em "Novo Comunicado" para comecar.'}
          </p>
        </div>
      ) : (
...
{!ann.read && (
                      <button
                        ...
                        aria-label={`Marcar comunicado "${ann.title}" como lido`}
                      >
...
{isAdminOrCoord && (
                      <button
                        ...
                        className="p-1.5 rounded-lg border border-brand-terciar/15 bg-brand-principal/20 text-brand-secundar hover:text-brand-secundar/80 hover:bg-brand-principal/60 cursor-pointer"
                        title="Editar"
                        aria-label={`Editar comunicado "${ann.title}"`}
                      >
...
{isAdmin && (
                      <button
                        ...
                        title="Excluir"
                        aria-label={`Excluir comunicado "${ann.title}"`}
                      >
